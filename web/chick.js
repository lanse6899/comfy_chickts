...
function renderImages() {
    ...
    container._clickHandler = async (e) => {
        ...
        // 如果只是单击（没有按任何修饰键），显示元数据或预览
        if (!isShiftPressed && !isCtrlPressed) {
            // 如果在预览模式下，立即显示预览，不需要延迟判断
            if (previewMode) {
                const img = item.querySelector('img');
                if (img) {
                    showPreviewImage(imgId, img);
                }
                // 如果元数据面板是显示的，也更新元数据
                const metadataPanel = document.getElementById('chick-metadata-panel');
            ...
            if (container) {
                ...
                if (item) {
                    ...
                    if (img) {
                        ...
                        showPreviewImage(firstSelectedId, img);
                    }
                }
            }
        }
...
function showPreviewImage(imageId, imageElement) {
    ...
    if (!previewWindow || !previewImage) return;
    ...
    previewImage.onload = function() {
        ...
        if (imgAspectRatio > maxAspectRatio) {
            // 图片较宽，以宽度为准
            previewWidth = Math.min(imgWidth, maxWidth);
            ...
            // 图片较高，以高度为准
            previewHeight = Math.min(imgHeight, maxHeight);
