...
function createBrowser() {
        ...
        ">
            ...
            <div style="display: flex; gap: 8px; align-items: center;">
                ...
                <button id="chick-delete-selected-btn" title="删除选中" style="
                    ...
                    align-items: center;
                    justify-content: center;
                " onmouseover="this.style.background='rgba(231,76,60,0.9)'" onmouseout="this.style.background='rgba(231,76,60,0.8)'">🗑️</button>
                <button id="chick-export-btn" title="导出选中" style="
                    ...
                    align-items: center;
                    justify-content: center;
                " onmouseover="this.style.background='rgba(76,175,80,0.9)'" onmouseout="this.style.background='rgba(76,175,80,0.8)'">📤</button>
                <button id="chick-save-btn" title="保存选中" style="
                    ...
                    align-items: center;
                    justify-content: center;
                " onmouseover="this.style.background='rgba(255,152,0,0.9)'" onmouseout="this.style.background='rgba(255,152,0,0.8)'">💾</button>
    ...
    if (metadataToggleBtn) {
        ...
        if (metadataPanel) {
            if (savedState) {
                ...
                metadataToggleBtn.style.background = 'rgba(74, 144, 226, 0.3)';
    ...
    // 绑定粘贴按钮事件
    document.getElementById('chick-paste-btn').addEventListener('click', async () => {
        try {
                ...
                for (const item of items) {
                    for (const type of item.types) {
                        if (type.startsWith('image/')) {
                            ...
                            const file = new File([blob], `pasted_${Date.now()}.png`, { type: type });
            ...
            // 如果新API不可用或没有图片，提示用户使用Ctrl+V
            showTempNotification('💡 请使用 Ctrl+V 粘贴图片');
            
            // 尝试触发粘贴事件（需要焦点在content上）
...
function renderImages() {
    ...
    container.innerHTML = imageStorage.map((img, index) => {
        ...
        <div data-id="${img.id}" data-index="${index}" data-image='${JSON.stringify(imageMeta).replace(/'/g, "&#39;")}' draggable="true" style="position: relative; background: #2a2a2a; border-radius: 6px; overflow: hidden; cursor: pointer; display: flex; align-items: center; justify-content: center; border: ${isSelected ? '3px solid #4a90e2' : '3px solid transparent'}; transition: border-color 0.2s;" class="chick-image-item">
            <div style="width: 100%; min-height: 120px; max-height: 200px; display: flex; align-items: center; justify-content: center; background: #1e1e1e;">
                <img src="${img.dataUrl}" style="max-width: 100%; max-height: 200px; object-fit: contain; display: block; pointer-events: none;" />
            </div>
        </div>
        `;
    }).join('');
...
function updateButtonStates() {
    ...
    if (exportBtn) {
        if (hasSelection) {
            if (exportBtn.style.display !== 'flex') {
...
async function saveToFileStorage() {
    try {
        const imagesToSave = imageStorage.map(img => ({
            id: img.id,
            name: img.name || `chick_${img.id}.png`,
            dataUrl: img.dataUrl,
            size: img.size || 0,
...
function applyImageToNode(node, imagePath, imageDataUrl) {
    ...
    if (node.widgets && node.widgets.length > 0) {
        for (let widget of node.widgets) {
            if (widget.name === 'image' || widget.name === 'filename' || 
                widget.type === 'image' || widget.name.toLowerCase().includes('image')) {
    ...
    if (imageDataUrl) {
        ...
        img.onload = () => {
            try {
                if (node.setImage) node.setImage(img);
                if (node.image) node.image = img;
                if (node.img) node.img = img;
...
function findNodeAtPosition(app, x, y) {
    ...
    if (app.graph._nodes && Array.isArray(app.graph._nodes)) {
        for (const node of app.graph._nodes) {
            ...
            const [w, h] = node.size || [200, 120]; // 提供一个保守的默认尺寸
...
function registerCanvasDrop() {
    ...
    canvas.addEventListener('dragover', function(e) {
        // 检查拖拽类型（通过types数组，而不是getData）
        if (e.dataTransfer.types && e.dataTransfer.types.includes('application/json')) {
            e.preventDefault();
...
function toggleMetadataPanel() {
    ...
    if (isVisible) {
        ...
        metadataPanel.style.display = 'block';
        toggleBtn.style.background = 'rgba(74, 144, 226, 0.3)';
