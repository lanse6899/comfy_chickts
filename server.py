...
try:
    ...
    print("[Chick] python-docx 未安装，DOCX解析功能不可用")
...
class ChickServer:
    ...
    def ensure_config_file(self):
        ...
        if not self.config_file.exists():
            ...
            default_config = {
                ...
                'show_cleanup_logs': False  # 默认隐藏清理日志
            }
            with open(self.config_file, 'w', encoding='utf-8') as f:
                json.dump(default_config, f, ensure_ascii=False, indent=2)
        else:
            # 如果配置文件已存在，检查并更新配置项（如果未设置）
            try:
                with open(self.config_file, 'r', encoding='utf-8') as f:
                    ...
                    if 'use_file_storage' not in config or config.get('use_file_storage') is False:
                        ...
                        updated = True
                    if 'show_cleanup_logs' not in config:
                        config['show_cleanup_logs'] = False  # 默认隐藏清理日志
                        updated = True
                    if updated:
                        with open(self.config_file, 'w', encoding='utf-8') as f:
    ...
    def _get_document_metadata(self, file_bytes, extension):
        """从文档文件中提取元数据"""
        info = {
            "file_type": extension.lower().lstrip('.'),
            "file_size": len(file_bytes),
            "file_size_readable": self._format_file_size(len(file_bytes))
        }

        try:
            # 解析 TXT 文件
            if extension.lower() in ['.txt', '.md', '.json', '.yaml', '.yml', '.xml', '.html', '.htm', '.css', '.js', '.py']:
                try:
                    # 尝试多种编码
                    for encoding in ['utf-8', 'gbk', 'gb2312', 'big5', 'latin1']:
                        ...
                        info["text_content"] = file_bytes.decode('utf-8', errors='ignore')
                        info["text_preview"] = info["text_content"][:500]
                except Exception as e:
                    print(f"[Chick] 解析TXT文件失败: {e}")

            # 解析 DOCX 文件
            elif extension.lower() == '.docx' and DOCX_AVAILABLE:
                try:
                    ...
                    info["text_preview"] = full_text[:500] if len(full_text) > 500 else full_text
                    ...
                    if tables_text:
                        info["tables_count"] = len(tables_text)
                        info["tables_preview"] = '\n\n'.join(tables_text)[:500]

                    # 统计信息
                    info["word_count"] = len(full_text.split())
                    info["char_count"] = len(full_text)
                    ...
                    if core_props.created:
                        info["created_at"] = str(core_props.created)
                    if core_props.modified:
                        info["modified_at"] = str(core_props.modified)

                except Exception as e:
                    print(f"[Chick] 解析DOCX文件失败: {e}")
                    info["parse_error"] = str(e)

            # 解析 DOC 文件（需要win32com，但通常是可选的）
            elif extension.lower() == '.doc':
                info["warning"] = "DOC格式需要额外依赖，请另存为DOCX格式以获得完整预览"
                # 尝试读取DOC文件的文本（可能不完整）
                try:
                    # 尝试使用 antiword（Linux）或其它工具
                    # 这里只做简单的二进制检查
                    info["is_official_doc"] = file_bytes[:4] == b'\xd0\xcf\x11\xe0'
                except:
                    pass

        except Exception as e:
            print(f"[Chick] 解析文档元数据失败: {e}")
            info["error"] = str(e)
    ...
    async def get_document_metadata(self, request):
        ...
        try:
            body = await request.json()
            base64_data = body.get('document_data', '')
            ...
            metadata = self._get_document_metadata(file_bytes, extension)
            ...
            print(f"[Chick] Error getting document metadata: {e}")
            import traceback
            traceback.print_exc()
            return web.json_response({
                'success': False,
        ...
        try:
            ...
            if extension in ['.txt', '.md', '.py', '.js', '.html', '.css', '.json', '.xml']:
                try:
                    ...
                    result['content'] = f'解析文本文件失败: {str(e)}'

            # 解析 DOCX 文件
            elif extension == '.docx' or extension.endswith('.docx'):
                if DOCX_AVAILABLE:
                    try:
                        doc = DocxDocument(io.BytesIO(file_bytes))
                        ...
                        # 限制内容长度
                        max_preview_size = 100 * 1024
                        if len(full_text) > max_preview_size:
                            full_text = full_text[:max_preview_size]
                            result['truncated'] = True
                        ...
                        for table_idx, table in enumerate(doc.tables):
                            ...
                            if table_content:
                                tables_text.append(f'--- 表格 {table_idx + 1} ---\n' + '\n'.join(table_content))

                        if tables_text:
                            result['tables'] = '\n\n'.join(tables_text)
                            result['tables_count'] = len(tables_text)
                        ...
                        result['content'] = f'解析DOCX文件失败: {str(e)}'
                        result['success'] = False
                else:
                    result['content'] = 'python-docx 库未安装，无法解析DOCX文件。\n\n请运行: pip install python-docx'
                    result['needs_dependency'] = True

            # 解析 DOC 文件（旧版Word格式）
            elif extension == '.doc':
                # 检测文件头：检查是否是有效的 OLE2 二进制格式（真正的 DOC）
                # DOC 文件的标志是 D0 CF 11 E0 A1 B1 1A E1
                is_valid_doc = len(file_bytes) > 8 and file_bytes[:4] == b'\xd0\xcf\x11\xe0'
                
                if is_valid_doc:
                    # 真正的旧版 DOC 二进制格式，Mammoth 无法处理
                    result['content'] = '''━━━━━━━━━━━━━━━━━━━━━━━━━━━
旧版 DOC 格式说明
━━━━━━━━━━━━━━━━━━━━━━━━━━━

DOC 是 Microsoft Word 97-2003 的二进制格式，
Mammoth 库无法直接解析此格式。
...
1. 将文档另存为 DOCX 格式：
   用 Microsoft Word 打开 → 文件 → 另存为 → DOCX

2. 或使用在线转换工具：
   • cloudconvert.com/doc-to-docx
   • convertio.co/doc-docx/

3. 或使用 WPS 另存为 DOCX

━━━━━━━━━━━━━━━━━━━━━━━━━━━
提示：建议将重要文档转换为 DOCX 格式，
      以获得更好的兼容性和安全性。
━━━━━━━━━━━━━━━━━━━━━━━━━━━'''
                    result['format'] = 'doc_legacy'
                    result['is_ole_binary'] = True
                else:
                    # 文件可能损坏或格式未知
                    result['content'] = '''━━━━━━━━━━━━━━━━━━━━━━━━━━━
文件格式说明
...
1. 检查文件扩展名是否正确
2. 尝试用 Microsoft Word 打开文件
3. 如有必要，请重新保存为 DOCX 格式
━━━━━━━━━━━━━━━━━━━━━━━━━━━'''
                    result['success'] = False
            ...
            print(f"[Chick] Error getting document content: {e}")
            import traceback
            traceback.print_exc()
            return web.json_response({
                'success': False,
        ...
        try:
            ...
            for img_data in images:
                try:
                    ...
                    mime = img_data.get('mime')
                    if not mime:
                        # 尝试从dataUrl推断
                        if img_data.get('dataUrl', '').startswith('data:'):
                            mime = img_data['dataUrl'].split(';')[0].replace('data:', '')
                        else:
                            # 根据扩展名兜底
                    ...
                    if media_type not in ['image', 'video', 'document']:
                        if mime.startswith('image/'):
                            ...
                            if 'word' in mime or mime in ['application/msword']:
                                media_type = 'document'
                            else:
                                media_type = 'document'
                        else:
            ...
            # 检查是否显示清理日志
            config = self.load_config()
            show_cleanup_logs = config.get('show_cleanup_logs', False)
            ...
            if deleted_count > 0 and show_cleanup_logs:
            ...
            for img_info in index_data.get('images', []):
                ...
                if file_path.exists():
                    try:
                        ...
                        print(f"[Chick] 加载媒体失败 {file_path}: {e}")
                        continue
    ...
    async def load_images_from_folder(self, request):
        """从指定文件夹加载所有图片"""
        try:
            body = await request.json()
            ...
            for file_path in folder.iterdir():
                ...
                try:
                    ...
                    # 生成唯一ID
                    file_id = f'folder_{int(time.time() * 1000)}_{loaded_count}_{hash(file_path.name) % 100000}'
                    ...
                    print(f"[Chick] 加载文件失败 {file_path}: {e}")
                    failed_count += 1
                    continue
            ...
            return web.json_response({
                ...
                'data': {
                    ...
                    'failed_count': failed_count,
                    'total_files': loaded_count + failed_count
                }
            })
            
        except Exception as e:
            print(f"[Chick] 从文件夹加载图片失败: {e}")
            import traceback
            traceback.print_exc()
            return web.json_response({
                'success': False,
